...
| **Dev 3 (State & Integration)** | Zustand, React Query e Refatoração | Configuração de `@bytebank/stores` (Zustand), `@bytebank/api-client` (Query hooks) e migração/remoção completa das Context APIs.          |
...
| **Dev 3** | Spike (1d) + Stores Zustand (1d) + Hooks Query (2d) + Migração Context (2.5d) + Smoke Test (0.5d)    | **7.0 dias** |
...
1. **Dia 1 (Todos)**: Spike de alinhamento em Zustand + TanStack Query (**Task 1**).
...
3. **Dias 4 e 5 (Dev 1, Dev 2 & Dev 3)**:
   ...
   - **Dev 3** cria as stores Zustand (**Task 7**).
...
> **Atenção (Mitigação de Riscos)**: O Dev 2 e o Dev 1 devem trabalhar em estreita colaboração nos dias 4 e 5. A entrega do NextAuth Setup pelo Dev 2 é necessária para que o Dev 3 consiga conectar a store de autenticação (`useAuthStore`) ao estado real do NextAuth. Caso ocorram atrasos, o Dev 3 deve programar mocks para as sessões e continuar a implementação.
